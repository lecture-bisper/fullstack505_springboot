...
//          현재 시간을 기준으로 새 파일명 생성, nanoTime()을 사용하여 동일한 이름이 나올 수 없도록 함, 위에서 생성한 확장자명하고 문자열을 연결함
...
//          DB에 저장하기 위한 BoardFileDto 클래스 타입의 객체에 파일 정보 데이터 추가
          ...
          // 서버에 저장되는 파일 이름, 위에서 생성한 파일 저장 경로와 nanoTime()을 이용하여 생성한 파일이름을 합하여 파일을 저장할 전체 경로를 생성함
